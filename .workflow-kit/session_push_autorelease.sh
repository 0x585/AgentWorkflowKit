...
# Source profile/file id: .workflow-kit/session_push_autorelease.sh
...
usage() {
  cat <<'USAGE'
Usage:
  ./.workflow-kit/session_push_autorelease.sh [--source-branch <codex/*>] [--target <branch>]

Examples:
  ./.workflow-kit/session_push_autorelease.sh
  ./.workflow-kit/session_push_autorelease.sh --source-branch codex/my-task --target <default-branch>
USAGE
}
...
SYNC_INFO="$("$ROOT/.workflow-kit/session_sync_status.sh" --porcelain "$TARGET_BRANCH")"
