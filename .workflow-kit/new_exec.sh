...
# Source profile/file id: .workflow-kit/new_exec.sh
...
if [[ -x "$ROOT/.workflow-kit/ensure_shared_venv.sh" ]]; then
  "$ROOT/.workflow-kit/ensure_shared_venv.sh" --quiet || true
fi
...
if [[ "$branch" == "$TARGET_BRANCH" ]]; then
  ...
  echo "Create one first: ./.workflow-kit/new_branch.sh \"issue words\"" >&2
  exit 1
fi
...
if [[ -f "$record_file" ]]; then
  ...
  echo "If this is intentional, use that ID directly; otherwise rerun ./.workflow-kit/new_exec.sh after cleanup." >&2
  exit 1
fi
