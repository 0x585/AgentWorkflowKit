...
# Source profile/file id: .workflow-kit/session_sync.sh
...
usage() {
  cat <<'USAGE'
Usage:
  ./.workflow-kit/session_sync.sh [--dry-run] [target_branch]

Examples:
  ./.workflow-kit/session_sync.sh
  ./.workflow-kit/session_sync.sh --dry-run
  ./.workflow-kit/session_sync.sh <default-branch>
USAGE
}
