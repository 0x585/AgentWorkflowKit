...
- auto-release 若因主仓 dirty 或 merge conflict 阻塞，应优先使用 `./.workflow-kit/session_release_resume.sh` 恢复，而不是在主仓手工补提交流程；如果只是想在发布前线性化 source branch 历史，可手动运行 `./.workflow-kit/session_sync.sh <default-branch>`。
